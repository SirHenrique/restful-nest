...
export class PedidoService {
  ...
  private trataDadosDoPedido(
    ...
    dadosDoPedido.itensPedido.forEach((itemPedido) => {
      ...
      if(produtoRelacionado === undefined) {
        throw new NotFoundException(`O produto com id ${itemPedido.produtoId} não foi encontado`)
      }

      if(itemPedido.quantidade > produtoRelacionado.quantidadeDisponivel) {
        throw new BadRequestException(`A quantidade solicitada (${itemPedido.quantidade}), é maior do que a quantidade disponível (${produtoRelacionado.quantidadeDisponivel})
        para o produto ${produtoRelacionado.id}`)
      }
    })
  }
  ...
  async atualizaPedido(pedidoId: string, dadosAtualizado: AtualizaPedidoDTO) {
    ...
    Object.assign(pedido, dadosAtualizado);
  ...
  async obtemPedidosDeUsuario(usuarioId: string) {
    ...
    if (usuario === null) {
      throw new NotFoundException('Não foi encontrado usuário informado!');
    }
